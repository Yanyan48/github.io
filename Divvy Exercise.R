...
# In what way s do members and casual riders use Divvy bikes differently?
...
setwd("/Users/anna/Desktop/Divvy_Exercise/csv") #set your working directory to
#simplify calls to data
...
# perfectly before we use a command to join them into one file.
colnames(q2_2019)
colnames(q3_2019)
colnames(q4_2019)
colnames(q1_2020)

# Rename column to make them consistent with q1_2020(as this will be the supposed
...
#Covert ride_id and rideable_type to character so that they can stack correctly
...
# Stack indidual quarter's data frames into one big data frame
all_trips <- bind_rows(q2_2019, q3_2019, q4_2019, q1_2020)

# Remove lat, long, birthyear, and gender fields as this data was dropped begining
# in 2020 
all_trips <- all_trips %>%  
...
summary(all_trips) #Statistic summary of data. Mainly for numeric

# There are a few problems we will need to fix:
# (1) In the "member_casual" column, there are two names for members("member"
# and subscriber") and two names for casual riders("Customer" and "casual"). We
# will need to consolidate that from four to two labels.
...
# (3) We will want to add a calculated field for length of ride since 2020Q1 data
# did not have the "tripduration" column. We will add "ride_length" to the entire
...
# with "casual"
# Before 2020, Divvy used different labels for these two types of rides... we
# will want to make our data frame consistent with their current nomenclature.
# N.B.: "Level" is a special property of a column that is retained even if a
# subset dose not contain any values from a special level.
# Begin by seeing how many obsevations fall under each usertype
...
# before completing these operation we could only aggregate at the ride level
all_trips$date <- as.Date(all_trips$started_at) # The default format is yyyy-mm-dd
all_trips$month <- format(as.Date(all_trips$date), "%m")
all_trips$day <- format(as.Date(all_trips$date),"%d")
all_trips$year <- format(as.Date(all_trips$date), "%Y")
all_trips$day_of_week <-format(as.Date(all_trips$date), "%A")
# Add a "ride_length" calculate to all_trips(in seconds)
...
#=================================================
# STEP 5:EXPLORT SUMMARY FILE FOR FURTHER ANNLYSIS
#=================================================
# Create a csv rile that we will visualize in Excel, Tableau, or my presentation
# software
